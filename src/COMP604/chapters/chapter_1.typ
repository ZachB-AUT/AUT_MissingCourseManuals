...
can save your extension for this assignment then great, but dont feel bad about
...
editor instead of having to use either copy them in/out or having to use a
...
+ Next, ensure the Homebrew package manager is installed.
  ...
  ```
  (If you dont already use homebrew, you should. Its extremely useful.)
...
+ Then, use Homebrew to install the risc-v compiler toolchain:
  ...
  ```
  With this step, sometimes people (me, I had this issue) have issues with compiling the riscv toolchain.
  If that occurs try changing your OSX version to OSX Sonoma, as there is a precompiled binary available for that version.
  Once thats done try running those commands again, Homebrew will handle it for you.
...
+ Next you must add the toolchain directory to your "path", which is where your computer looks for programs if they are not in the current directory.
  ...
  # setup saying the computer cant find the compiler toolchain or something,
...
If that throws an error, try separating all of the packages into seperate lines in a text file (Lets call it packs.txt), and running the following command:
```sh
...
+ Reboot your computer, open bios and ensure that hardware virtualisation is enabled. (look up how to do this, it varies by motherboard manufacturer)
